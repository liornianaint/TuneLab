...
class UnifiedColorCheckerUISmokeTests(unittest.TestCase):
    ...
    def test_one_workspace_contains_csv_image_and_original_cc_features(self) -> None:
        ...
        self.assertEqual(
            ...
            ["色差对比", "图像对比", "工程统计", "诊断与解释", "History / XML Diff"],
        )
        ...
        self.assertEqual(self.app.image_solver_var.get(), "统一 ΔE/ΔC/Δh 优化（推荐）")
        tool_labels = [
    ...
    def test_image_end_to_end_uses_shared_optimizer_and_renders_simulation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
        ...
        self.assertIn("ΔE00", self.app.image_metrics_var.get())
    ...
    def test_profile_is_advanced_and_cannot_bypass_patch_protection(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            measured_path = root / "4000K_Before.png"
            xml_path = root / "cc13.xml"
            ...
            self.app.load_test_image(str(measured_path))
            self.app.load_xml(str(xml_path))
            self.app.image_solver_var.set("3000K/4000K 实拍 Profile（安全门禁）")
            self.app.run_optimization()

        self.assertIsNotNone(self.app.result)
        self.assertFalse(any(patch.regression_status == "FAIL" for patch in self.app.result.patch_results))
        self.assertTrue(
            self.app.result.search_method.startswith("calibrated-restoration")
            or any("Profile 未通过" in warning for warning in self.app.result.warnings)
        )
