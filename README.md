...
TuneLab 当前正式版本为 `1.1.0`。每次启动都会静默检查仓库最新的正式 GitHub Release；“帮助 → 检查更新...”可随时手动检查。发现更高版本后，TuneLab 会询问是否打开对应 Release 页面，由用户下载适合 Windows 或 macOS 的安装包并手动安装；程序不会自行覆盖当前可执行文件。断网或尚未发布 Release 时，自动检查保持静默，手动检查会显示明确结果。Release 标签应使用可比较的版本格式，例如 `v1.1.1`、`v1.2.0`。
