...
启动器会在工程根目录创建或复用 `.venv`，使用该虚拟环境的 Python 启动 TuneLab。首次运行以及 `pyproject.toml` 依赖变化时，会自动执行一次可编辑安装；普通启动不会重复安装依赖。TuneLab 窗口显示后会在后台访问 GitHub Release API 检查新版本，不阻塞启动。Windows 使用 `py run_tunelab.py`。
...
构建脚本会强制使用工程 `.venv`，只打包程序运行依赖，不包含 `sources/` 测试素材；签名后会直接启动一次成品进行自检。缺少 NumPy/OpenCV 等运行库时构建会立即失败，不再生成无法启动的伪成品。产物为 `dist/TuneLab.app`。
...
TuneLab 当前正式版本为 `1.0.0`。每次启动都会静默检查仓库最新的正式 GitHub Release；“帮助 → 检查更新...”可随时手动检查。发现更高版本后，TuneLab 会询问是否打开对应 Release 页面，由用户下载适合 Windows 或 macOS 的安装包并手动安装；程序不会自行覆盖当前可执行文件。断网或尚未发布 Release 时，自动检查保持静默，手动检查会显示明确结果。Release 标签应使用可比较的版本格式，例如 `v1.0.1`、`v1.1.0`。
