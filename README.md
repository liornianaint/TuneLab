...
TuneLab 是一个面向 Qualcomm Camera 的跨平台调试工作台。目前包含统一的 CCM / ColorChecker 校正、Gamma15 和图像分析器三个工作区。CCM 页面既可读取 Imatest CSV，也可直接从色卡图片自动取样：CSV 使用带回退保护的工程多目标优化，图片输入默认直接逼近标准/自定义 ColorChecker 的 24 色块；两者共用报告、History 和定点 XML 回写。

作者联系邮箱：<kaiyi.jiang@thundersoft.com>

当前版本包含：

- Windows / macOS 共用一套原生 macOS 风格 Tk 桌面界面；macOS 使用原生 Aqua 控件与系统 San Francisco，Windows 优先使用 Segoe UI Variable，并为矩阵与 Diff 使用平台等宽字体；首次启动默认适配当前屏幕
...
- 图像画布和图库均支持右键菜单，可设定对比基准、旋转、水平/垂直镜像、还原方向及重命名；旋转和镜像只改变当前查看方向，不写回图片内容
- 类 Finder/ImageViewer 的批量重命名：可逐张勾选、实时预览，默认格式为序号在前的 `{n}_{name}`，并支持 `{name}` / `{n}` 占位符、起始序号与位数设置、扩展名保留、冲突预检和可回滚的两阶段改名
...
启动器会在工程根目录创建或复用 `.venv`，使用该虚拟环境的 Python 启动 TuneLab。首次运行以及 `pyproject.toml` 依赖变化时，会自动执行一次可编辑安装；普通启动不会重复安装依赖。源码运行时会在后台访问 GitHub Release API 检查新版本，不阻塞启动。Windows 使用 `py run_tunelab.py`。
...
工程根目录的 `sources/` 仅作为开发和测试素材。源码运行时，CCM、Gamma 与图像分析器的原生文件选择窗口会优先从该目录打开；构建产物不会把这些素材打包进 APP，旧的单数目录名不再使用。
...
构建脚本会强制使用工程 `.venv`，只打包程序运行依赖，不包含 `sources/` 测试素材；macOS 包会下载并校验固定版本的 Sparkle 2、嵌入原生自动更新框架，再对完整应用签名。签名后会直接启动一次成品，并验证 Sparkle 控制器可以创建。缺少 NumPy/OpenCV 等运行库或 Sparkle 校验失败时，构建会立即停止，不再生成无法启动的伪成品。产物为 `dist/TuneLab.app`。
...
TuneLab 当前正式版本为 `1.3.0`。从 `1.2.0` 开始，打包的 macOS APP 会由 Sparkle 2 每天在后台检查 GitHub 最新正式 Release，自动下载、验证 EdDSA 签名，并在安全时原子替换应用；`1.1.0` 及更早版本需要先手动安装一个新版本。“帮助 → 检查更新...”会直接打开原生更新窗口，不再跳转浏览器。

源码运行、Windows 包或缺少 Sparkle.framework 的异常 macOS 包会继续使用 GitHub Release 检查作为安全后备，并引导用户手动下载。Release 标签应使用可比较的版本格式，例如 `v1.2.1`、`v1.3.0`。

macOS 正式发布使用：

```bash
.venv/bin/python scripts/prepare_macos_release.py
```

该命令会构建并验证 APP，生成 `dist/TuneLab-<版本>-macOS-arm64.zip`，同时使用登录钥匙串中账户 `com.tunelab.app` 的私钥签名更新包与 `appcast.xml`。私钥不会写入工程；`appcast.xml` 和 ZIP 必须一起作为同一个 GitHub Release 的资源上传。应用固定访问 `releases/latest/download/appcast.xml`，所以 Release 应先以草稿形式上传两个资源，再正式发布。若已配置 Apple Developer ID，可通过 `TUNELAB_CODESIGN_IDENTITY` 指定签名身份；否则构建脚本使用本机临时签名。
...
5. CSV 模式可设置 Strategy、Regularization、饱和度系数、重点 Patch、权重与系数边界；图片模式固定使用 24 色块等权目标，只保留系数范围和 Matrix Health 等数值约束。参数会自动写入内部 `settings.json`。
...
7. 检查“工程统计”和“诊断与解释”：Matrix Health 不得为 FAIL。CSV 模式还要求 Pass Rate 不下降、重点 Patch 不得明显退化；图片模式以 24 色块平均 ΔE00 是否下降为准，局部色块回退仅作提示。
...
图片输入使用单独的标准色卡逼近目标：测试图的 24 个 measured sRGB 直接与标准/自定义目标的 24 个 sRGB 比较，并等权最小化平均 CIEDE2000 ΔE00。它不做逐色块亮度替换，也不以单色块回退、Pass Rate、重点 Patch、Strategy、Regularization、强度或饱和度作为候选门禁；最终矩阵仍须满足系数范围、Neutral 轴、可逆性和 Matrix Health 非 FAIL。由于 JPEG 中的曝光、Gamma、Tone Mapping 等误差也会被计入，该模式是“向目标图逼近”，不是从 JPEG 严格反推 Camera RGB 绝对矩阵。
...
3. 图片色块直接与标准/自定义目标的 24 个 sRGB 色值比较，所有色块等权最小化平均 ΔE00；不再逐块匹配测试亮度。曝光、Gamma、Tone Mapping 或 JPEG 裁切造成的差异也会计入目标距离。
...
6. 图片模式固定调用标准色卡逼近求解器，不再选择 3000K/4000K 实拍 Profile，也不执行 Patch Regression、Pass Rate、重点 Patch、策略、正则、强度或饱和度门禁；局部回退会显示为 WARNING，但不会否决平均 ΔE00 更低的候选。
7. “ColorChecker 输入”并排等宽显示原始测试图、改后模拟图和标准/自定义目标。改后图始终在线性 sRGB 中应用本轮实际 Delta CCM，因此与 XML 使用同一矩阵；它是近似预览，不能复现完整 RAW→JPEG ISP，也不能替代烧录后的上机重拍。
8. “保存 XML”再次确认后覆盖当前加载的原文件，仅替换所选 Region 的 9 个 `<c_tab><c>` 数值，并立即回读校验。CSV 模式仍以 Matrix/Patch 保护决定是否可写；图片模式只在 Matrix Health=FAIL 或没有有效矩阵变化时禁用。
...
- 可像桌面图片查看器一样使用“打开图片…”或 `Ctrl/⌘+O` 直接载入 1–4 张图；也可“打开文件夹”。文件夹路径显示在可编辑地址栏中；打开文件夹后，左侧会出现类似 macOS“照片”的纵向图库，缩略图只在进入可视范围时后台解码。首次只打开自然顺序中的第一张，不再自动选中 4 张。
- 空画布中央的加号及整块提示区域可直接打开图片；地址栏左侧的主页按钮会清空当前图片、图库、选区和分析结果并返回未打开状态，但仍保留上次目录供下一次原生文件对话框使用。
- 图库普通点按为单选；`⌘/Ctrl+点按` 可按目录顺序增减多选，`Shift+点按` 可连续选择，最多 4 张。“上一组 / 下一组”或 `Alt+← / Alt+→` 按当前选择数量沿自然顺序移动。左栏内的 `‹` 和地址栏“显示图库 / 收起图库”可一键释放或恢复图片空间。
- 界面只使用中性的“图像 1–4”和实际文件名，不根据打开顺序预设文件语义。1 张铺满画布，2 张左右排列，3 张等宽水平排列，4 张使用 2×2 布局。
- 工作区采用“左侧图库—中间图片—右侧检查器”的三栏结构，并沿用首页的 macOS 字体、浅色表面、细分隔线和系统蓝选中态。顶部合并为紧凑的双行工具栏，不再重复展示大标题和说明文字；图库与检查器的默认宽度也经过收窄，将更多横向和纵向空间留给图片。右栏只保留精简对比表以及“直方图 / EXIF”页；每张活动图片都可独立显示或隐藏整图/选区直方图与 EXIF，整个右栏也可一键收起。首页布局和样式不受图像分析器改动影响。
...
- 点按任一图片会按归一化坐标同步取样全部活动图片；左键拖动建立选区并同步统计。中间画布底部只显示每张图的 `R / G / B / R/G / R/B`：点按后显示像素值，建立选区后显示对应选区均值。每次打开或切换分组时，每张图都会独立适应自己的画布，并在窗口尺寸变化时保持适应，直到用户主动缩放、1:1 或平移。中键、右键或 Shift+左键拖动画布时，现有画面立即跟随指针移动，并以约 60 Hz 合并高质量裁片刷新；滚轮、触控板及工具栏“− / +”会以同一归一化锚点联动缩放全部当前图片。
- 文件名、尺寸、位深、解码精度和缩放倍率作为画布内浮层显示，不占用图片布局高度；图片区域始终使用完整画布空间。
- 显示缓存只渲染当前可见裁片，并复用相同裁片的缩放结果；大图适应窗口时使用一个有界的显示预览层，放大到 1:1 后再从原图提取可见区域。普通 8-bit 图片共享分析/显示数组；像素、ROI、直方图和匹配始终读取完整原图数据，显示优化不改变任何分析公式或精度。
- 完整图片使用按文件时间校验、受条目数和内存预算约束的 LRU 缓存。普通 8-bit 解码不建立全图 float64 副本；大图采用单路后台解码，避免全新环境首次并发打开 19 MP 图片时产生瞬时内存峰值。返回刚查看过的分组通常无需重复解码，后台加载/统计/匹配不会阻塞 Tk 主线程。
- 在图片画布或左侧图库上右键，可旋转、镜像、还原方向、在图库中定位及重命名。工具栏、文件菜单或 `⇧⌘R`（Windows 为 `Ctrl+Shift+R`）可打开当前文件夹的批量重命名；可在列表中逐张勾选本次要处理的图片，默认按 `{n}_{name}` 生成“序号_原名”，也可用占位符自由改写格式。改名前会显示完整的新旧名称列表并检查重复名与已存在目标，扩展名和图片内容保持不变。

详细像素文本与独立结论页不再占用界面。底部保留最常用的 RGB 与通道比例，精简对比表仍用于 A/B 数值检查；右侧逐图直方图会在建立选区后自动从整图切换到对应选区。

所有普通 8-bit RGB 默认按标准 sRGB 处理：先执行 sRGB 传递函数解码到 Linear RGB，再转换到 D65 XYZ 与 CIE Lab；相对亮度也在线性 RGB 上计算。“检查器 → 对比”另外显示标准 sRGB/Rec.709 权重计算的绝对亮度 Y′（0–255）、A/B 差值和百分比变化，便于直接核对最终图片的 8-bit 明暗值；它不是以 cd/m² 表示的物理亮度。HSV 的 S/V 使用 0–1 范围，H 使用角度。接近中性的像素不会仅因某一通道略大就被描述为明显偏色，暖/冷/红/绿方向判断会避开过暗与可能剪切的区域。

### 多图 ROI 匹配

1. 选择 2–4 张图片后，可在任意一张图中框选至少 5×5 原图像素的 ROI；首次框选的图片成为本组选区锚点。
2. 工具把锚点 ROI 分别映射到其余图片；若尺寸不同，会按各图宽高比例映射中心与尺寸，不假设像素坐标完全一致。
...
4. 其余图片分别独立显示最佳 ROI、匹配分数和置信度。自动匹配后仍可直接在任一非锚点图片上重新框选并手动调整；再次调整锚点则重新匹配其余图片。
...
多图对比页可分别选择任意“图像 A”和“图像 B”，也可一键交换 Delta 方向。当 A/B 都不是选区锚点时，数值门禁使用两者匹配置信度的较低值。精简表格显示选区平均色、A/B 数值、Delta 与变化方向，变化列统一使用带符号的百分数，不再附加详细文本、解释段落或独立结论。

### CSV、配置与数据边界

“导出当前分析 CSV”使用 UTF-8 BOM，按图像 1 和最多三张当前图像逐行写入图片尺寸/位深、ROI 坐标、匹配分数、RGB/HSV/Lab/亮度/剪切/暗部指标，以及相对图像 1 的绝对 RGB、RGB 百分比和 RGB 占比 Delta 字段。默认只写文件名以降低路径隐私泄露；可在“导出”菜单显式启用完整路径。

图像分析器单独保存上次目录、搜索范围、匹配阈值、直方图默认开关、默认选区名称和路径导出开关；首次打开默认适配当前屏幕，不会自动保存图片像素、选区结果或检查器内容。
...
在目标系统上安装 PyInstaller。macOS 开发构建运行：
...
macOS 正式发布运行 `python3 scripts/prepare_macos_release.py`。Windows 则在 Windows 上运行 `python scripts/build_windows.py`。

产物位于 `dist/`：

- Windows：`dist/TuneLab.exe`
