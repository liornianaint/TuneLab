...
TuneLab 当前正式版本为 `1.2.0`。`1.1.0` 及更早版本需手动安装一次 `1.2.0`；从 `1.2.0` 开始，打包的 macOS APP 会由 Sparkle 2 每天在后台检查 GitHub 最新正式 Release，自动下载、验证 EdDSA 签名，并在安全时原子替换应用。“帮助 → 检查更新...”会直接打开原生更新窗口，不再跳转浏览器。

源码运行、Windows 包或缺少 Sparkle.framework 的异常 macOS 包会继续使用 GitHub Release 检查作为安全后备，并引导用户手动下载。Release 标签应使用可比较的版本格式，例如 `v1.1.1`、`v1.2.0`。
