...
- 独立 Qualcomm Gamma15 模块：Imatest Stepchart、连续有效灰阶识别、257 点 R/G/B LUT 优化与安全 XML 回写
...
从“工具 → Gamma 优化...”打开独立页面，也可运行 `python3 -m matrixcorrect.gamma_app`。Gamma 模块与 CC 流程隔离：
...
3. 仅连续可区分的灰阶参加目标 Gamma、Local Gamma 和灰阶误差拟合；不可区分、非连续、近黑或过曝点被排除。
4. 优化当前 Region 的 R/G/B 257 点、0–1023 整数 LUT。默认 RGB 联动，仅改变亮度响应；高级模式允许受 RGB 色偏门禁约束的独立微调。
5. 多目标 Loss 包含灰阶目标、Local Gamma、LUT 平滑、原 LUT 变化、高光/暗部保护与 RGB 灰阶偏差。结果必须单调不下降、无局部反转、无异常突变并保持首尾点。
6. 保存使用“另存为”，默认 `原文件名_optimized.xml`；只替换当前 Region 的三个 LUT，其余 Region 保持不变并执行回读校验。
...
Golden Regression 会去重遍历 `Source` / `source` 中的全部文件，并按格式路由：CCM Golden 使用兼容的 ColorChecker CSV/CC XML；Gamma Regression 使用 Stepchart CSV/Gamma XML。当前 8 个 CCM 光源/照度 Case（A、CWF、D65、TL84）必须全部满足：Average ΔE 改善、至少一个 `ΔE00≤2/3/5/10` Pass Rate 提升、Neutral 19–24 无明显退化、无 FAIL Patch、无工程 FAIL、无越界 Matrix、Row Sum=1 且饱和度偏差不扩大。
...
- Qualcomm `80-74889-81_REV_AH_Qualcomm_Spectra_1080_Deep_Dive.pdf` 第 414-417 页：CC1.3 / CC1.4 与 Gamma1.5 IPE trigger/color-format 映射。
