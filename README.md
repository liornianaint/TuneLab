...
- Windows / macOS 共用一套原生 macOS 风格 Tk 桌面界面；macOS 使用系统 San Francisco，Windows 优先使用 Segoe UI Variable，并为矩阵与 Diff 使用平台等宽字体
...
1. **CC CSV（Imatest）**：沿用原工作流，读取 measured/ideal RGB。若另外导入过测试图，切回 CSV 后仍可用 CSV 求解并查看该实拍图的整图仿真。
...
5. 图片模式默认“统一 ΔE/ΔC/Δh 优化（推荐）”。此前的 3000K/4000K 实拍矩阵保留为高级 Profile：完整强度必须通过同一套 Patch Regression、ΔC/Δh、Neutral、Pass Rate 和 Matrix 门禁；失败时自动回退强度，仍无安全候选则自动改用统一优化，不允许绕过门禁写回。
6. “图像对比”显示原始测试图、整图仿真和标准/自定义目标。这里的 JPEG ΔE00 明确标记为“不参与求解”；可写回判断以“色差对比”页的亮度匹配 ΔE/ΔC/Δh 和工程保护为准。仿真可导出，但不能替代烧录后的重拍验证。
7. “保存 XML”再次确认后覆盖当前加载的原文件，仅替换所选 Region 的 9 个 `<c_tab><c>` 数值，并立即回读校验。Matrix FAIL、Patch FAIL 或没有有效矩阵变化时按钮不可用。

高级 Profile 被安全接受时，预览继续使用 `3000K_Before → 3000K_After`、`4000K_Before → 4000K_After` 的实拍响应；统一优化使用 linear-sRGB Delta CCM 预览。传感器、曝光或 ISP 配置改变后，两种预览都需要重新上机拍摄确认。
