...
7. “ColorChecker 输入”并排显示原始测试图、改后仿真图和标准/自定义目标。安全接受 Profile 时使用实拍 Before→After 响应模型还原预览，其余情况使用 linear-sRGB Delta CCM 近似；两类预览均可导出，但不参与求解/写回门禁，也不能替代烧录后的上机重拍。
...
- 可像桌面图片查看器一样使用“打开图片…”或 `Ctrl/⌘+O` 直接载入 1–4 张图；也可“打开文件夹”，在左侧缩略图列表中 Ctrl/⌘ 多选后点击“载入所选”，双击同样可载入当前选择。
...
- 鼠标移动实时显示原图坐标 RGB；点击固定取样点；左键拖动建立 ROI。中键、右键或 Shift+左键拖动画布；滚轮、触控板及工具栏“− / +”会以同一归一化锚点联动缩放全部当前图片，“适应窗口”和“1:1”可快速恢复视图。
...
图像分析器单独保存上次目录、搜索范围、匹配阈值、直方图/实时像素开关、默认 ROI 名称、中性模式、文件夹预览栏比例和路径导出开关；窗口每次按当前屏幕尺寸展开，不会自动保存图片像素、ROI 结果或分析结论。
