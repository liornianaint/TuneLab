...
所有普通 8-bit RGB 默认按标准 sRGB 处理：先执行 sRGB 传递函数解码到 Linear RGB，再转换到 D65 XYZ 与 CIE Lab；相对亮度也在线性 RGB 上计算。HSV 的 S/V 使用 0–1 范围，H 使用角度。接近中性的像素不会仅因某一通道略大就被描述为明显偏色，暖/冷/红/绿方向判断会避开过暗与可能剪切的区域。
