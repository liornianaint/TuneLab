...
- `optimizer.py`：多目标 Loss、自动正则化、边界搜索、Regression/Saturation Protection
- `engineering.py`：系数范围、Row Sum、Condition、Determinant、Rank、Smoothness、Q-format 仿真
- `diagnostics.py`：Gamma/AWB/CC/SCE/2D LUT/CV 的 Confidence 与 Root Cause
- `report.py`：可审计的 CSV / HTML / PDF / Excel 报告
- `settings.py`、`history.py`：跨平台参数与 Matrix History 持久化
- `golden.py`：Source 全量发现、去重、验收和 JSON/HTML 汇总
...
`python3 -m matrixcorrect.golden --source Source` 会遍历所有 CSV × XML，并对每个 Case 验证：
